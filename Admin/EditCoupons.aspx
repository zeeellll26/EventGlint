...
<head runat="server">
    <title>Edit Coupons — EventGlint</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .sidebar-nav::-webkit-scrollbar{width:3px;}.sidebar-nav::-webkit-scrollbar-thumb{background:var(--border);border-radius:10px;}
        ...
        .form-card{background:var(--card);border:1px solid var(--border);border-radius:14px;box-shadow:var(--shadow-sm);overflow:hidden;transition:box-shadow 0.22s;}.form-card:hover{box-shadow:var(--shadow-md);}
        ...
        .radio-item{display:flex;align-items:center;gap:8px;font-size:13.5px;color:var(--sub);cursor:pointer;}
        ...
        tbody tr:last-child{border-bottom:none;}tbody tr:hover{background:rgba(91,63,248,0.03);}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="EditCoupons.aspx" class="nav-item active"><i class="fas fa-percent"></i> Coupons</a>
...
<div class="main">
    <div class="topbar">
        <div class="topbar-left"><h2>Edit Coupons</h2><p>Manage discount codes and offers</p></div>
    ...
    <div class="content">
        <div class="welcome-banner fade-in">
            <div class="greeting">🏷️ Coupon Management</div>
            <h1>Manage Coupons</h1>
            <p>Create discount codes, set limits and manage active offers for your users.</p>
        </div>
        <div class="section-title">Add / Edit Coupon</div>
        <div class="body-grid">
            <div class="form-card fade-in">
                <div class="form-card-head">
                    <div class="form-card-icon"><i class="fas fa-percent"></i></div>
                    <div><div class="form-card-title">Coupon Details</div><div class="form-card-sub">Fill in details and click Insert or Save</div></div>
                </div>
                <div class="form-card-body">
                    <asp:Label ID="lblMessage" runat="server" Visible="false" />
                    <div class="field">
                        <label><i class="fas fa-lock" style="opacity:0.4;margin-right:4px"></i>Coupon ID</label>
                        <asp:TextBox ID="txtCouponId" runat="server" ReadOnly="true" placeholder="Auto generated" />
                    </div>
                    <div class="field">
                        <label>Coupon Code <span class="req">*</span></label>
                        <asp:TextBox ID="txtCode" runat="server" placeholder="e.g. SAVE20" />
                    </div>
                    <div class="field">
                        <label>Discount Type <span class="req">*</span></label>
                        <div class="radio-group">
                            <label class="radio-item"><asp:RadioButton ID="rbPercentage" runat="server" GroupName="DiscountType" Text="Percentage" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbFlat"       runat="server" GroupName="DiscountType" Text="Flat" /></label>
                        </div>
                    </div>
                    <div class="field">
                        <label>Discount Value <span class="req">*</span></label>
                        <asp:TextBox ID="txtDiscountValue" runat="server" placeholder="e.g. 20" TextMode="Number" />
                    </div>
                    <div class="field">
                        <label>Max Discount</label>
                        <asp:TextBox ID="txtMaxDiscount" runat="server" placeholder="e.g. 150" TextMode="Number" />
                    </div>
                    <div class="field">
                        <label>Min Order Value</label>
                        <asp:TextBox ID="txtMinOrder" runat="server" placeholder="e.g. 300" TextMode="Number" />
                    </div>
                    <div class="field">
                        <label>Valid From <span class="req">*</span></label>
                        <asp:TextBox ID="txtValidFrom" runat="server" TextMode="Date" />
                    </div>
                    <div class="field">
                        <label>Valid Till <span class="req">*</span></label>
                        <asp:TextBox ID="txtValidTill" runat="server" TextMode="Date" />
                    </div>
                    <div class="field">
                        <label>Usage Limit</label>
                        <asp:TextBox ID="txtUsageLimit" runat="server" placeholder="Leave blank = unlimited" TextMode="Number" />
                    </div>
                    <div class="field">
                        <label>Used Count</label>
                        <asp:TextBox ID="txtUsedCount" runat="server" placeholder="0" TextMode="Number" />
                    </div>
                    <div class="btn-row">
            ...
            <div class="table-card fade-in">
                <div class="table-card-head">
                    <h3><i class="fas fa-table" style="margin-right:8px;color:var(--accent)"></i>All Coupons</h3>
                    <span class="count-badge"><asp:Label ID="lblCount" runat="server" Text="0 records" /></span>
                </div>
                <div class="tbl-wrap">
                    <asp:GridView ID="gvCoupons" runat="server" AutoGenerateColumns="false" DataKeyNames="CouponId" GridLines="None" EmptyDataText="No coupons found." OnRowCommand="gvCoupons_RowCommand">
                        <Columns>
                            ...
                            <asp:BoundField DataField="ValidFrom"     HeaderText="From" DataFormatString="{0:dd-MM-yyyy}" />
                            <asp:BoundField DataField="ValidTill"     HeaderText="Till" DataFormatString="{0:dd-MM-yyyy}" />
                            <asp:BoundField DataField="UsedCount"     HeaderText="Used" />
                            <asp:TemplateField HeaderText="Actions">
                                <ItemTemplate>
                                    <asp:Button CommandName="SelectRow" CommandArgument='<%# Eval("CouponId") %>' Text="Edit"   CssClass="tbtn tbtn-select" runat="server" />
                                    <asp:Button CommandName="DeleteRow" CommandArgument='<%# Eval("CouponId") %>' Text="Delete" CssClass="tbtn tbtn-delete" runat="server" OnClientClick="return confirm('Delete this coupon?');" />
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
