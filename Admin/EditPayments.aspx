<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EditPayments.cs" Inherits="EventGlint.Admin.EditPayments" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Edit Payments — EventGlint</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .sidebar-nav{flex:1;overflow-y:auto;padding:14px 10px;}.sidebar-nav::-webkit-scrollbar{width:3px;}.sidebar-nav::-webkit-scrollbar-thumb{background:var(--border);border-radius:10px;}
        ...
        .nav-item:hover{background:var(--accent-light);color:var(--accent);}.nav-item:hover i{color:var(--accent);}
        .nav-item.active{background:linear-gradient(135deg,rgba(91,63,248,0.12),rgba(192,38,211,0.06));color:var(--accent);font-weight:600;}.nav-item.active i{color:var(--accent);}
        .sidebar-footer{padding:14px 10px;border-top:1px solid var(--border);}
        .logout-btn{display:flex;align-items:center;gap:10px;width:100%;padding:10px 14px;border-radius:8px;background:#fff1f1;border:1px solid #fecaca;color:var(--danger);font-size:13.5px;font-weight:600;font-family:'DM Sans',sans-serif;cursor:pointer;text-decoration:none;transition:all 0.18s;}.logout-btn:hover{background:#ffe4e4;border-color:#f87171;}
        ...
        .form-card{background:var(--card);border:1px solid var(--border);border-radius:14px;box-shadow:var(--shadow-sm);overflow:hidden;}.form-card:hover{box-shadow:var(--shadow-md);}
        ...
        .form-card-title{font-size:14px;font-weight:600;color:var(--text);}.form-card-sub{font-size:11.5px;color:var(--muted);margin-top:2px;}
        .form-card-body{padding:22px;}
        .field{margin-bottom:15px;}.field label{display:block;font-size:11px;font-weight:600;color:var(--sub);text-transform:uppercase;letter-spacing:0.6px;margin-bottom:6px;}
        .req{color:var(--danger);margin-left:2px;}
        .field input,.field select,.field textarea{width:100%;padding:9px 13px;border:1.5px solid var(--border);border-radius:8px;font-size:13.5px;color:var(--text);background:var(--bg);font-family:'DM Sans',sans-serif;transition:all 0.2s;}
        .field input:focus,.field select:focus,.field textarea:focus{outline:none;border-color:var(--accent);background:white;box-shadow:0 0 0 3px rgba(91,63,248,0.10);}
        ...
        .radio-group{display:flex;flex-wrap:wrap;gap:12px;padding:4px 0;}.radio-item{display:flex;align-items:center;gap:7px;font-size:13px;color:var(--sub);cursor:pointer;}.radio-item input[type=radio]{accent-color:var(--accent);width:14px;height:14px;}
        ...
        .btn:hover{transform:translateY(-1px);filter:brightness(1.08);}.btn:active{transform:translateY(0);}
        ...
        .btn-clear{background:var(--bg);color:var(--sub);border:1.5px solid var(--border);}.btn-clear:hover{border-color:var(--accent);color:var(--accent);background:var(--accent-light);}
        ...
        .msg-success{background:#d1fae5;border:1px solid #6ee7b7;color:#065f46;}.msg-error{background:#fee2e2;border:1px solid #fca5a5;color:#991b1b;}
        .table-card{background:var(--card);border:1px solid var(--border);border-radius:14px;box-shadow:var(--shadow-sm);overflow:hidden;}.table-card:hover{box-shadow:var(--shadow-md);}
        ...
        .tbl-wrap{overflow-x:auto;}table{width:100%;border-collapse:collapse;}thead tr{background:rgba(91,63,248,0.04);}
        ...
        tbody tr{border-bottom:1px solid rgba(228,232,240,0.6);transition:background 0.14s;}tbody tr:last-child{border-bottom:none;}tbody tr:hover{background:rgba(91,63,248,0.03);}
        tbody td{padding:12px 14px;font-size:13px;color:var(--sub);white-space:nowrap;}
        .tbtn{padding:5px 12px;border-radius:7px;font-size:11.5px;font-weight:600;cursor:pointer;border:none;font-family:'DM Sans',sans-serif;transition:all 0.15s;}.tbtn:hover{transform:translateY(-1px);}
        .tbtn-select{background:var(--accent-light);color:var(--accent);border:1px solid rgba(91,63,248,0.2);}.tbtn-select:hover{background:rgba(91,63,248,0.15);}
        .tbtn-delete{background:#fff1f1;color:var(--danger);border:1px solid #fecaca;margin-left:4px;}.tbtn-delete:hover{background:#fee2e2;}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="EditPayments.aspx"    class="nav-item active"><i class="fas fa-credit-card"></i> Payments</a>
...
<div class="main">
    <div class="topbar">
        <div class="topbar-left"><h2>Edit Payments</h2><p>View and manage all payment transactions</p></div>
    ...
    <div class="content">
        <div class="welcome-banner fade-in">
            <div class="greeting">💳 Payment Management</div>
            <h1>Payment</h1>
        ...
        <div class="body-grid">
            <div class="form-card fade-in">
                <div class="form-card-head">
                    ...
                    <div><div class="form-card-title">Payment Details</div><div class="form-card-sub">Fill in details and click Insert or Save</div></div>
                </div>
                <div class="form-card-body">
                    <asp:Label ID="lblMessage" runat="server" Visible="false" />
                    <div class="field"><label><i class="fas fa-lock" style="opacity:0.4;margin-right:4px"></i>Payment ID</label>
                        <asp:TextBox ID="txtPaymentId" runat="server" ReadOnly="true" placeholder="Auto generated" /></div>
                    <div class="field"><label>Booking <span class="req">*</span></label>
                        <asp:DropDownList ID="ddlBookingId" runat="server" /></div>
                    <div class="field"><label>Amount <span class="req">*</span></label>
                        <asp:TextBox ID="txtAmount" runat="server" placeholder="e.g. 500.00" TextMode="Number" /></div>
                    <div class="field"><label>Payment Method <span class="req">*</span></label>
                        <div class="radio-group">
                            <label class="radio-item"><asp:RadioButton ID="rbCC"  runat="server" GroupName="PM" Text="CreditCard" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbDC"  runat="server" GroupName="PM" Text="DebitCard" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbUPI" runat="server" GroupName="PM" Text="UPI" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbNB"  runat="server" GroupName="PM" Text="NetBanking" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbWallet" runat="server" GroupName="PM" Text="Wallet" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbCOD" runat="server" GroupName="PM" Text="COD" /></label>
                        </div></div>
                    <div class="field"><label>Gateway <span class="req">*</span></label>
                        <div class="radio-group">
                            <label class="radio-item"><asp:RadioButton ID="rbRazorpay" runat="server" GroupName="GW" Text="Razorpay" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbStripe"   runat="server" GroupName="GW" Text="Stripe" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbPayU"     runat="server" GroupName="GW" Text="PayU" /></label>
                        </div></div>
                    <div class="field"><label>Gateway Txn ID</label>
                        <asp:TextBox ID="txtGatewayTxnId" runat="server" placeholder="e.g. pay_RZP001" /></div>
                    ...
                    <div class="field"><label>Paid At</label>
                        <asp:TextBox ID="txtPaidAt" runat="server" TextMode="DateTimeLocal" /></div>
                    <div class="field"><label>Refunded At</label>
                        <asp:TextBox ID="txtRefundedAt" runat="server" TextMode="DateTimeLocal" /></div>
                    <div class="field"><label>Refund Amount</label>
                        <asp:TextBox ID="txtRefundAmount" runat="server" placeholder="e.g. 200.00" TextMode="Number" /></div>
                    <div class="btn-row">
            ...
            <div class="table-card fade-in">
                <div class="table-card-head">
                    <h3><i class="fas fa-table" style="margin-right:8px;color:var(--accent)"></i>All Payments</h3>
                    <span class="count-badge"><asp:Label ID="lblCount" runat="server" Text="0 records" /></span>
                </div>
                <div class="tbl-wrap">
                    <asp:GridView ID="gvData" runat="server" AutoGenerateColumns="false" DataKeyNames="PaymentId" GridLines="None" EmptyDataText="No records found." OnRowCommand="gvData_RowCommand">
                        <Columns>
                            <asp:BoundField DataField="PaymentId"     HeaderText="ID" />
                            <asp:BoundField DataField="BookingRef"    HeaderText="Booking Ref" />
                            <asp:BoundField DataField="Amount"        HeaderText="Amount" DataFormatString="Rs.{0:N0}" />
                            <asp:BoundField DataField="PaymentMethod" HeaderText="Method" />
                            <asp:BoundField DataField="Gateway"       HeaderText="Gateway" />
                            <asp:BoundField DataField="Status"        HeaderText="Status" />
                            <asp:BoundField DataField="PaidAt"        HeaderText="Paid At" DataFormatString="{0:dd-MM-yyyy}" />
                            <asp:TemplateField HeaderText="Actions">
                                <ItemTemplate>
                                    <asp:Button CommandName="SelectRow" CommandArgument='<%# Eval("PaymentId") %>' Text="Edit"   CssClass="tbtn tbtn-select" runat="server" />
                                    <asp:Button CommandName="DeleteRow" CommandArgument='<%# Eval("PaymentId") %>' Text="Delete" CssClass="tbtn tbtn-delete" runat="server" OnClientClick="return confirm('Delete this record?');" />
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
