...
<head runat="server">
    <title>Edit Halls — EventGlint</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .radio-group{display:flex;flex-wrap:wrap;gap:12px;padding:4px 0;}
        .radio-item{display:flex;align-items:center;gap:7px;font-size:13px;color:var(--sub);cursor:pointer;}
        ...
        table{width:100%;border-collapse:collapse;}thead tr{background:rgba(91,63,248,0.04);}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="EditHalls.aspx"          class="nav-item active"><i class="fas fa-door-open"></i> Halls</a>
...
<div class="main">
    <div class="topbar">
        <div class="topbar-left"><h2>Edit Halls</h2><p>Manage screens and auditoriums within venues</p></div>
    ...
    <div class="content">
        <div class="welcome-banner fade-in">
            <div class="greeting">📽️ Hall Management</div>
            <h1>Manage Halls</h1>
            <p>Add screens and auditoriums to venues, set hall type and total capacity.</p>
        </div>
        <div class="section-title">Add / Edit Hall</div>
        <div class="body-grid">
            <div class="form-card fade-in">
                <div class="form-card-head">
                    <div class="form-card-icon"><i class="fas fa-door-open"></i></div>
                    <div><div class="form-card-title">Hall Details</div><div class="form-card-sub">Fill in details and click Insert or Save</div></div>
                </div>
                <div class="form-card-body">
                    <asp:Label ID="lblMessage" runat="server" Visible="false" />
                    <div class="field"><label><i class="fas fa-lock" style="opacity:0.4;margin-right:4px"></i>Hall ID</label>
                        <asp:TextBox ID="txtHallId" runat="server" ReadOnly="true" placeholder="Auto generated" /></div>
                    <div class="field"><label>Venue Name <span class="req">*</span></label>
                        <asp:DropDownList ID="ddlVenueId" runat="server" /></div>
                    <div class="field"><label>Hall Name <span class="req">*</span></label>
                        <asp:TextBox ID="txtHallName" runat="server" placeholder="e.g. Screen 1" /></div>
                    <div class="field"><label>Hall Type <span class="req">*</span></label>
                        <div class="radio-group">
                            <label class="radio-item"><asp:RadioButton ID="rb2D"   runat="server" GroupName="HallType" Text="2D" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rb3D"   runat="server" GroupName="HallType" Text="3D" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbIMAX" runat="server" GroupName="HallType" Text="IMAX" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rb4DX"  runat="server" GroupName="HallType" Text="4DX" /></label>
                            <label class="radio-item"><asp:RadioButton ID="rbDolby" runat="server" GroupName="HallType" Text="Dolby" /></label>
                        </div>
                    </div>
                    <div class="field"><label>Total Capacity <span class="req">*</span></label>
                        <asp:TextBox ID="txtCapacity" runat="server" placeholder="e.g. 200" TextMode="Number" /></div>
                    <div class="btn-row">
            ...
            <div class="table-card fade-in">
                <div class="table-card-head">
                    <h3><i class="fas fa-table" style="margin-right:8px;color:var(--accent)"></i>All Halls</h3>
                    <span class="count-badge"><asp:Label ID="lblCount" runat="server" Text="0 records" /></span>
                </div>
                <div class="tbl-wrap">
                    <asp:GridView ID="gvHalls" runat="server" AutoGenerateColumns="false" DataKeyNames="HallId" GridLines="None" EmptyDataText="No halls found." OnRowCommand="gvHalls_RowCommand">
                        <Columns>
                            <asp:BoundField DataField="HallId"       HeaderText="ID" />
                            <asp:BoundField DataField="VenueName"    HeaderText="Venue" />
                            <asp:BoundField DataField="Name"         HeaderText="Hall Name" />
                            <asp:BoundField DataField="HallType"     HeaderText="Type" />
                            <asp:BoundField DataField="TotalCapacity" HeaderText="Capacity" />
                            <asp:TemplateField HeaderText="Actions">
                                <ItemTemplate>
                                    <asp:Button CommandName="SelectRow" CommandArgument='<%# Eval("HallId") %>' Text="Edit"   CssClass="tbtn tbtn-select" runat="server" />
                                    <asp:Button CommandName="DeleteRow" CommandArgument='<%# Eval("HallId") %>' Text="Delete" CssClass="tbtn tbtn-delete" runat="server" OnClientClick="return confirm('Delete this hall?');" />
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
